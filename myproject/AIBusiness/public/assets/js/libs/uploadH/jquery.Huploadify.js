(function($) {
	$.fn.Huploadify = function(opts) {
		...
		var itemTemp = '';//<div id="${fileID}" class="uploadify-queue-item">上传中</div>
		var defaults = {
			...
			fileSizeLimit: 4048, //允许上传的文件大小，单位KB
			showUploadedPercent:false, //是否实时显示上传的百分比，如20%
			showUploadedSize: false, //是否实时显示已上传的文件大小，如1M/2M
			buttonText: '选择文件', //上传按钮上的文字
			removeTimeout: 5500, //上传完成后进度条的消失时间
			itemTemplate:itemTemp, //上传队列显示的模板itemTemp
			onUploadStart: false, //上传开始时的动作
			onUploadSuccess: true, //上传成功的动作
			onUploadComplete:true, //上传完成的动作
			onUploadError: true, //上传失败的动作
			onInit: null, //初始化时的动作
		...
		this.each(function() {
			...
			inputStr += '"/>';
			inputStr += '<a id="file_upload_' + instanceNumber + '-button" href="javascript:void(0)" class="uploadify-button vasn">';
						...
						xhr.onreadystatechange = function(e) {
							if(xhr.readyState == 4) {
								if(xhr.status == 200) {
									...
									thisfile.find('.uploadify-progress-bar').css('width', 'percent');
